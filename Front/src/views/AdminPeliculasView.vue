...
import GeneroService from '../services/GeneroService'; // Necesitamos los géneros para el select
...
// Data
const peliculas = ref([]);
const allGeneros = ref([]); // Lista de géneros disponibles para el formulario
...
const form = ref({
  ...
  precio: 0.00,
  estaActivo: true,
  generosIds: [] // Array de IDs [1, 5]
});

onMounted(async () => {
  await loadGeneros(); // Cargar géneros primero
  loadPeliculas(0);
});
...
const loadPeliculas = async (page) => {
  loading.value = true;
  try {
    const data = await PeliculaService.getAll(page, 5);
    peliculas.value = data.content;
    currentPage.value = data.number;
    totalPages.value = data.totalPages;
  } catch (error) {
    errorMsg.value = "Error cargando películas";
  } finally {
    loading.value = false;
  }
};
...
const openCreateModal = () => {
  ...
  form.value = {
    ...
    anioLanzamiento: 2024, duracionMinutos: 90,
    imgPortada: '', imgBanner: '', urlStream: '',
    precio: 0.00, estaActivo: true, generosIds: []
  };
  showModal.value = true;
};

const openEditModal = (peli) => {
  isEditing.value = true;
  // Mapeamos los datos para el formulario
  form.value = {
    ...peli,
    // Importante: Convertir la lista de objetos Genero a lista de IDs para los checkboxes
    generosIds: peli.generos.map(g => g.id)
  };
  showModal.value = true;
};
...
const savePelicula = async () => {
  try {
    if (isEditing.value) {
      await PeliculaService.update(form.value.id, form.value);
    } else {
      await PeliculaService.create(form.value);
    }
    showModal.value = false;
    loadPeliculas(currentPage.value);
  } catch (error) {
...
// Paginación
const prevPage = () => { if (currentPage.value > 0) loadPeliculas(currentPage.value - 1); };
const nextPage = () => { if (currentPage.value < totalPages.value - 1) loadPeliculas(currentPage.value + 1); };
</script>
    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="flex justify-between items-center mb-8">
        <div>
          ...
          <p class="text-gray-400 text-sm mt-1">Gestiona el catálogo de contenido</p>
        </div>
        <div class="flex gap-3">
          ...
          <button @click="openCreateModal" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg shadow-lg flex items-center gap-2">
          ...
          <table class="w-full text-left">
            <thead class="bg-gray-900/50 text-gray-400 uppercase text-xs font-semibold">
              <tr>
                ...
                <th class="p-4">Año</th>
                <th class="p-4">Géneros</th>
                <th class="p-4">Estado</th>
                <th class="p-4 text-right">Acciones</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-700">
              <tr v-for="peli in peliculas" :key="peli.id" class="hover:bg-gray-700/30 transition">
                <td class="p-4">
                  <img :src="peli.imgPortada" class="h-16 w-10 object-cover rounded bg-gray-900" alt="Cover"/>
                </td>
                <td class="p-4">
                  <p class="font-bold text-white">{{ peli.titulo }}</p>
                  <p class="text-xs text-gray-500 truncate w-40">{{ peli.slug }}</p>
                </td>
                <td class="p-4">{{ peli.anioLanzamiento }}</td>
                <td class="p-4">
                  <div class="flex flex-wrap gap-1">
                    <span v-for="g in peli.generos" :key="g.id" class="px-2 py-0.5 bg-gray-700 rounded text-xs text-gray-300">
                      {{ g.nombre }}
                    </span>
                  </div>
                </td>
                <td class="p-4">
                  <span :class="`px-2 py-1 rounded text-xs font-bold ${peli.estaActivo ? 'bg-green-900/30 text-green-400' : 'bg-red-900/30 text-red-400'}`">
                    {{ peli.estaActivo ? 'Público' : 'Borrador' }}
                  </span>
                </td>
                <td class="p-4 text-right space-x-2">
                  <button @click="openEditModal(peli)" class="text-blue-400 hover:text-blue-300">✏️</button>
                  <button @click="deletePelicula(peli.id)" class="text-red-400 hover:text-red-300">🗑️</button>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div class="bg-gray-800 rounded-2xl shadow-2xl w-full max-w-3xl border border-gray-700 my-10">
        ...
        <div class="p-6 border-b border-gray-700 flex justify-between items-center">
          <h2 class="text-xl font-bold text-white">{{ isEditing ? 'Editar Película' : 'Nueva Película' }}</h2>
          <button @click="showModal = false" class="text-gray-400 hover:text-white">✕</button>
        </div>

        <form @submit.prevent="savePelicula" class="p-6 space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input v-model="form.slug" required class="input-dark" placeholder="ej: batman-inicia" />
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <input v-model="form.duracionMinutos" type="number" class="input-dark" />
            </div>
            <div>
              <label class="label">Precio ($)</label>
              <input v-model="form.precio" type="number" step="0.01" class="input-dark" />
            </div>
          ...
          <div>
            <label class="label">URL Stream (.m3u8 / .mp4)</label>
            <input v-model="form.urlStream" placeholder="https://..." class="input-dark" />
          </div>
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-3 p-3 bg-gray-900/50 rounded-lg border border-gray-700 max-h-32 overflow-y-auto">
              <label v-for="g in allGeneros" :key="g.id" class="flex items-center gap-2 cursor-pointer px-2 py-1 hover:bg-gray-700 rounded transition">
                <input type="checkbox" :value="g.id" v-model="form.generosIds" class="rounded accent-blue-500" />
          ...
          <div class="flex justify-end gap-3 pt-4">
            ...
            <button type="submit" class="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold">Guardar</button>
          </div>
...
.input-dark { @apply w-full px-3 py-2 bg-gray-700 text-white border border-gray-600 rounded focus:outline-none focus:border-blue-500; }
</style>